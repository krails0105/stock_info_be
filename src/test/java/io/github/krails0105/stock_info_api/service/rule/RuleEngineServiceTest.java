 ...
 *
 * <ul>
 *   <li>룰 평가 순서: HF(하드필터) → M(과열/변동성) → N(뉴스) → P(실적) → V(밸류) → S(안정성)
 *   <li>템플릿 선택: E(리스크) > B(모멘텀) > D(성장) > A(밸류) > C(안정)
 *   <li>카드 슬롯: positive 2-3개, caution 1-2개, caution 최소 1개 보장
 * </ul>
 */
class RuleEngineServiceTest {
  ...
  class HardFilterTests {
    ...
    void testSuspendedStock() {
      StockSignals signals = baseSignals().isSuspended(true).build();
      ...
      assertThat(insight.getReasons().getCaution()).anyMatch(c -> c.getText().contains("거래정지"));
    }
    ...
    void testLowLiquidity() {
      ...
      assertThat(insight.getReasons().getCaution()).anyMatch(c -> c.getText().contains("유동성"));
    }
  ...
  class OverheatVolatilityTests {
    ...
    void testOverheatWithVolumeSpike() {
      ...
      assertThat(insight.getReasons().getCaution())
          .anyMatch(c -> c.getText().contains("급등") || c.getText().contains("과열"));
    }
  ...
  class NewsTests {
    ...
    void testPositiveNews() {
      ...
      StockSignals signals = baseSignals().newsItems(List.of(positiveNews)).build();

      StockInsight insight = ruleEngineService.buildStockInsight(signals);

      assertThat(insight.getReasons().getTriggeredRules()).contains("N-01");
      assertThat(insight.getReasons().getPositive())
          .anyMatch(c -> c.getText().contains("긍정") || c.getText().contains("촉매"));
    }

    @Test
    @DisplayName("N-02: 부정 뉴스(규제 리스크)는 E_RISK 템플릿 + 경고 카드")
    void testNegativeNews() {
      NewsItem negativeNews =
          NewsItem.builder()
      ...
      StockSignals signals = baseSignals().newsItems(List.of(negativeNews)).build();
      ...
      assertThat(insight.getReasons().getCaution())
          .anyMatch(c -> c.getText().contains("리스크") || c.getText().contains("변동성"));
    }
  }
  ...
  class FundamentalsTests {
    ...
    void testImprovingEarnings() {
      StockSignals signals = baseSignals().earningsTrend("IMPROVING").build();
      ...
      assertThat(insight.getReasons().getPositive()).anyMatch(c -> c.getText().contains("실적"));
    }

    @Test
    @DisplayName("P-02: 실적 둔화는 경고 카드 추가")
    void testDecliningEarnings() {
      StockSignals signals = baseSignals().earningsTrend("DECLINING").build();
  ...
  class ValuationTests {
    ...
    void testUndervaluedPer() {
      ...
      assertThat(insight.getReasons().getPositive())
          .anyMatch(c -> c.getText().contains("PER") || c.getText().contains("부담"));
    }
    ...
    void testOvervaluedPer() {
      ...
      assertThat(insight.getReasons().getCaution())
          .anyMatch(c -> c.getText().contains("기대") || c.getText().contains("조정"));
    }
  }
  ...
  class StabilityMomentumTests {
    ...
    void testLowVolatility() {
      ...
      assertThat(insight.getReasons().getPositive())
          .anyMatch(c -> c.getText().contains("변동성") || c.getText().contains("초보자"));
    }
  ...
  class CardSlotTests {
    ...
    void testPositiveCardDiversity() {
      ...
      long distinctCategories = positives.stream().map(ReasonCard::getCategory).distinct().count();
      // 최소 2개 이상의 다른 카테고리
  ...
  class ScoreGradeTests {
    ...
    void testConfidenceByDataCoverage() {
      // 높은 커버리지
      StockSignals highCoverage = baseSignals().dataCoverage(0.9).build();
      ...
      // 중간 커버리지
      StockSignals mediumCoverage = baseSignals().dataCoverage(0.6).build();
      ...
      // 낮은 커버리지
      StockSignals lowCoverage = baseSignals().dataCoverage(0.3).build();
  ...
  @Nested
  @DisplayName("P0-2: 헤드라인 톤 매칭 테스트")
  class ToneMatchingTests {

    @Test
    @DisplayName("강한 톤: score>=70 && coverage>=0.7 → 우선 검토/유력 후보")
    void testStrongTone() {
      // 높은 점수를 유도하기 위해 changeRate를 높게 설정 (score = (changeRate + 5) * 10)
      // changeRate=7 → score=120 (capped at 100), 모멘텀 템플릿
      StockSignals signals =
          baseSignals()
              .changeRate(7.0)
              .volumeRatio(1.8) // 모멘텀
      ...
      // 강한 톤 헤드라인에는 "우선순위가 높은" 또는 "유력 후보" 문구 포함
      assertThat(insight.getSummary().getHeadline()).containsAnyOf("우선", "유력");
    }

    @Test
    @DisplayName("중간 톤: 50<=score<70 → 관찰 리스트")
    void testMediumTone() {
      // score=55 유도: changeRate=0.5 → (0.5+5)*10 = 55
      StockSignals signals =
          baseSignals()
              .changeRate(0.5)
              .dataCoverage(0.8)
              .volatility(5.0)
              .sectorMedianVolatility(10.0) // 안정형 템플릿
              .build();

      StockInsight insight = ruleEngineService.buildStockInsight(signals);

      // 중간 톤 헤드라인에는 "관찰" 문구 포함
      assertThat(insight.getSummary().getHeadline()).contains("관찰");
    }

    @Test
    @DisplayName("조건부 톤: score<50 → 확인 후 접근")
    void testConditionalTone_LowScore() {
      // score=41 유도: changeRate=-0.9 → (-0.9+5)*10 = 41
      StockSignals signals =
          baseSignals()
              .changeRate(-0.9)
              .dataCoverage(0.8)
              .volatility(5.0)
              .sectorMedianVolatility(10.0)
              .build();

      StockInsight insight = ruleEngineService.buildStockInsight(signals);

      // 조건부 톤 헤드라인에는 "확인" 문구 포함
      assertThat(insight.getSummary().getHeadline()).contains("확인");
    }

    @Test
    @DisplayName("HF-04: 정보 부족 시 조건부 프리픽스 '정보가 제한적이지만'")
    void testHF04_ConditionalPrefix() {
      StockSignals signals =
          baseSignals()
              .changeRate(3.0) // 보통 점수
              .dataCoverage(0.5) // 낮은 커버리지 → HF-04 트리거
              .per(10.0)
              .sectorMedianPer(15.0)
              .build();
      ...
      assertThat(insight.getReasons().getTriggeredRules()).contains("HF-04");
      // 조건부 프리픽스 확인
      assertThat(insight.getSummary().getHeadline()).startsWith("정보가 제한적이지만");
    }
  }
}
